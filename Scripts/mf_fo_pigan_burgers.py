...
class Burgers_PIGAN():
    def __init__(self, x_u, y_u, x_f, X_star, u_star, G, D, Q, device, nepochs, lambda_phy=1.0, lambda_mse=0.0,
                 lambda_mse_d=1, lambda_q=1.5, lr_g = 1e-4, lr_d = 1e-4, noise=0.0):
        ...
        #         self.G = G.apply(weights_init)
        #         self.D = D.apply(weights_init)
        #         self.Q = Q.apply(weights_init)

        self.lr_g = lr_g
        self.lr_d = lr_d
        
        self.D_optimizer = torch.optim.Adam(self.D.parameters(), lr=self.lr_d, betas=(0.9, 0.999))
        params = list(self.G.parameters()) + list(self.Q.parameters())
        self.G_optimizer = torch.optim.Adam(params, lr=self.lr_g, betas=(0.9, 0.999))
    ...
    def generate_fake(self, X, noise):
        ...
        residual, u_x = self.get_residual(x, t, u)
        return residual, u, u_x
    ...
    def train_disriminator(self, x, y, Z_u):
        ...
        _, u, _ = self.generate_fake(x, Z_u)
        # u = torch.tensor(u.detach(), requires_grad=True).float().to(self.device)
        fake_logits_u = self.D(torch.cat([x, u[:, 0:1]], dim=1))
    ...
    def train_generator(self, x, y, Z_u, Z_f):
        ...
        residual_f, u_f, u_x_f = self.generate_fake(self.train_x_f, Z_f)

        # physics loss for boundary points
        _, y_pred, _ = self.generate_fake(x, Z_u)
        fake_logits_u = self.D(torch.cat([x, y_pred[:, 0:1]], dim=1))
        ...
        mse_loss_d = self.lambda_mse_d * torch.nn.functional.mse_loss(u_f[:, 1:2], u_x_f)
        mse_loss = mse_loss + mse_loss_d
