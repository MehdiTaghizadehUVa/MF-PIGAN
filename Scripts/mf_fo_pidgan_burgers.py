...
class Burgers_PIDGAN():
    def __init__(self, x_u, y_u, x_f, X_star, u_star, G, D, Q, device, nepochs, lambda_phy=1.0, lambda_mse=0.0,
                 lambda_mse_d=1, lambda_q=1.5, lambda_val = 0, noise=0.0):
        ...
        self.lambda_mse_d = lambda_mse_d
    ...
    def generate_fake(self, X, noise):
        ...
        residual, u_x_ex = self.phy_residual(x, t, u)
        n_phy = torch.exp(-self.lambda_val * (residual**2))
        return n_phy, residual, u, u_x_ex
    ...
    def predict(self, X):
        ...
        u = self.G(torch.cat([x, t, q, noise], dim=1))
        f, _ = self.phy_residual(x, t, u)
        u = u[:, 0:1].detach().cpu().numpy()
    ...
    def train_disriminator(self, x, y, Z_u, Z_f):
        ...
        n_phy, residual,y_pred, _= self.generate_fake(x, Z_u)
        fake_logits_u = self.D(torch.cat([x, y_pred[:, 0:1], residual], dim=1))

        # physics loss for collocation points
        n_phy_f, residual_f, u_f, _= self.generate_fake(self.train_x_f, Z_f)
        fake_logits_f = self.D(torch.cat([self.train_x_f, u_f[:, 0:1], residual_f], dim=1))
        ...
        real_logits_f = self.D(torch.cat([self.train_x_f, u_f[:, 0:1], real_prob_f], dim=1))
    ...
    def train_generator(self, x, y, Z_u, Z_f):
        # physics loss for collocation points
        n_phy1, residual_f, u_f, u_x_f = self.generate_fake(self.train_x_f, Z_f)
        fake_logits_f = self.D(torch.cat([self.train_x_f, u_f[:, 0:1], residual_f], dim=1))
#             fake_logits_f = self.D.forward(torch.cat([self.train_x_f, u_f[:, 0:1], n_phy1], dim=1))
        # physics loss for boundary points
        n_phy2, residual_u, y_pred, _ = self.generate_fake(x, Z_u)
        fake_logits_u = self.D(torch.cat([x, y_pred[:, 0:1], residual_u], dim=1))
#             fake_logits_u = self.D.forward(torch.cat([x, y_pred[:, 0:1], n_phy2], dim=1))
        ...
        mse_loss_u = torch.nn.functional.mse_loss(y_pred[:, 0:1], y)
        mse_loss_u_x = torch.nn.functional.mse_loss(u_f[:, 1:2], u_x_f)

        mse_loss = self.lambda_mse * mse_loss_u + self.lambda_mse_d * mse_loss_u_x
